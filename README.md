...
* **MIPC Transport Deadlock Elimination**: Repaired a critical architectural flaw where the EMW3080 Wi-Fi module silently stalled the entire STM32 SPI bus for exactly 10,000ms during TCP chunk downloads. The download sequence was rewritten: stripping the unsupported `MSG_DONTWAIT` (`0x08`) socket flag which crashed the MIPC firmware, and replacing the fragile 500ms blind SPI yield with a precise `100ms` `SO_RCVTIMEO` socket configuration. This guarantees safe, internal blocking inside the Wi-Fi module with instantaneous replies to the host, boosting OTA throughput and eliminating the deadly `0x0205` transport hangs.

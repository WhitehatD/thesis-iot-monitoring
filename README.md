...
* **MIPC Transport Deadlock Elimination**: Repaired a critical architectural flaw where the EMW3080 Wi-Fi module silently stalled the entire STM32 SPI bus for exactly 10,000ms during TCP chunk downloads. The MIPC socket protocol was rigorously audited: the AT firmware completely ignores the `SO_RCVTIMEO` socket option and unsupported `MSG_DONTWAIT` (`0x08`) flags, defaulting to infinite blocking on empty hardware receive buffers. The download sequence was rewritten to implement a robust POSIX `select()` polling mechanism (`MX_WIFI_Socket_select`) with a precise 100ms yield, guaranteeing that `recv` is never blindly called until data is explicitly available. This guarantees immediate architectural responsiveness to the host, boosting OTA throughput and permanently eliminating the deadly `0x0205` timeout transport hangs.

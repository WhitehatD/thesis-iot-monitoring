...
export default function DeviceStatusStepper({ jobState }) {
    ...
    return (
        <div className={`stepper-banner ${jobState?.isActive ? "visible" : "hidden"}`}>
            <div className="stepper-header">
                <span className="stepper-title">
                    {jobState?.step === "error" ? "🚨 Error" : "⚡ Live Board Progress"}
                </span>
                {jobState?.taskId && <span className="stepper-task">Task #{jobState.taskId}</span>}
            </div>
            
            {jobState?.step === "error" ? (
                <div className="stepper-error-msg">{jobState?.error || "Unknown Error"}</div>
            ) : (
                <div className="stepper-track">
                    {steps.map((step, idx) => (
                        <div key={step.id} className={`stepper-step-container ${getStepClass(idx)}`}>
                            <div className="stepper-step">
                                <div className="step-icon">{step.icon}</div>
                                <div className="step-label">{step.label}</div>
                            </div>
                            {idx < steps.length - 1 && (
                                <div className={`step-line ${idx < currentStepIndex ? "active-line" : ""}`} />
                            )}
                        </div>
                    ))}
                </div>

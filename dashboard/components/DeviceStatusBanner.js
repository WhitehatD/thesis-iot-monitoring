...
export default function DeviceStatusBanner({ deviceStatus }) {
	...
	const statusConfig = {
		...
		schedule_received: {
			icon: "📥",
			label: "Schedule successfully synced to board",
			color: "emerald",
			pulse: true,
		},
		...
		ota_rebooting: {
			icon: "🔁",
			label: "Rebooting with new firmware...",
			color: "amber",
			pulse: true,
		},
		schedule_cleared: {
			icon: "🗑️",
			label: "Schedule cleared from board",
			color: "slate",
		},
		error: {
			icon: "❌",
			label: `Error: ${deviceStatus?.reason || "unknown"}`,
			color: "red",
		},
	};
	...
	return (
		<div
			className={`device-status-banner ${cfg.color}${cfg.pulse ? " pulse" : ""}`}
		>
			...
			{deviceStatus.latency_ms != null && (
				<span className="device-status-latency">
					{deviceStatus.latency_ms}ms
				</span>
			)}
		</div>
	);

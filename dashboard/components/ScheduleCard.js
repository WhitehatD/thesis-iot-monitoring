 ...
 */
export default function ScheduleCard({ schedule, onActivate, onDelete, onRefresh }) {
    ...
    const handleActivate = async (e) => {
        ...
        setActivating(true);
        try {
            const res = await fetch(`${apiBase}/api/schedules/${schedule.id}/activate`, {
                method: "POST",
            });
                ...
                <div className="schedule-card-info">
                    ...
                    <div className="schedule-card-meta">
                        <span className="schedule-badge task-count-badge">
                            {schedule.tasks?.length || 0} task{(schedule.tasks?.length || 0) !== 1 ? "s" : ""}
                        </span>
                        {schedule.created_at && (
